defmodule KeyX do
  ...
  def generate_shares(k,n,secret, base \\ :binary, base_opts \\ [case: :lower]) do
    ...
    encoded_shares = case base do
      ...
      _other -> {:error, "no matching base"}
    end

    {:ok, base, encoded_shares}
  end
  ...
  def recover_secret(shares, base \\ :binary, base_opts \\ [case: :lower]) do
    decoded_shares = case base do
      :binary -> shares
      :base16 -> shares |> Enum.map(&(Base.decode16!(&1, base_opts)))
      :base32 -> shares |> Enum.map(&(Base.decode32!(&1, base_opts)))
      :base64 -> shares |> Enum.map(&(Base.decode64!(&1, base_opts)))
      _other -> {:error, "no matching base"}
    end
    {:ok, recover_secret!(decoded_shares)}
  end
